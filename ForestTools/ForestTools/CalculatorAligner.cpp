...
const std::string distanceMeasure::CalculatorAligner::create_sequence_set_aligned_file(AlignedDistanceMeasureCalculator* dmc, FileObjectManager& fileObjectManager, const std::vector<std::string>& sequence_set_names, const std::string& sequence_set) const
{
	...
	if(!aligned_file)
	{
		//check size -- if non-zero (eof?) do not recreate file
			//NOTE:: ASSUMPTION -- .afa files are at least 10 bytes
		//if (fseek(aligned_file, 10, SEEK_SET))//if file opened and is NOT greater than 10 bytes************
		//{
			//TIME ALIGNMENT
