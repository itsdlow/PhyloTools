...
{
	//REMOVE MIDDLE MAN -- MOVE COMMONALITIES BETWEEN ALIGNED AND UNALIGNED INTERNAL CALC --> DISTANCEMEASURECALCULATOR (base)
		//separate into 2 classes == minimize duplicate code (after)
	void InternalDistanceMeasureCalculator::calculate_and_output_matrix(FileObjectManager& fileObjectManager, const std::vector<std::string>& sequence_set_names, const std::string& sequence_set, const int batch_id)
	{
		...
		this->CalculateLargeTreeDistanceMeasures(this, fileObjectManager, sequence_set_names);
		this->CalculateAllQuartetsDistanceMeasures(this, fileObjectManager, sequence_set_names);

		////create file for current sequence_set
		//this->write_batch_results( this->GetCalculatorName(), batch_id, sequence_set_names.size() );

		//this->create_tree(this->GetCalculatorName(), sequence_set_names, batch_id);
	}
	...
	//					count++;
	//					//NOTE:: CHANGE TO '\r' when other debug print statements removed
	//					printf("%d/%d %s Quartet Matrices calculated...\n", count, totalQuartets, this->GetCalculatorName().c_str());
	//				}
	//			}
	//		}
	//	}
